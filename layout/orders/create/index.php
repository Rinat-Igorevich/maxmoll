...
<div class="container">
    ...
    <form action="/layout/orders/" method="post" >
        ...
        <div class="container" style="text-align: center">
            ...
            Статус заказа
            <select class="form-select" name="status" style="width: 150px; display: inline"  <?= $status ? '' : 'disabled' ?>>
        ...
        <table class="table" id="createOrder">
            <thead>
                ...
                <th>Скидка</th>
                <th>Сумма</th>
                <th><button type="button" id="addButton" class="btn  btn-dark" value="<?= isset($orderItems) ? count($orderItems) : 1?>" onclick="products.addRow(2)">+ строка</button></th>
                <th><button type="button" id="addButton" class="btn  btn-dark" value="1" onclick="products.deleteRow()">- строка</button></th>
            </thead>
            <tbody>
            ...
            <?php foreach ($orderItems as $key => $orderItem): ?>
                <tr id="<?= $key + 1 ?>">
                    <th><?= $key + 1 ?></th>
                    <td><select id="product<?= $key + 1 ?>" name="product_<?= $key + 1 ?>" required onchange="products.setStockAndPrice(event)">
                    ...
                    <td><input id="count<?= $key + 1 ?>" name="count_<?= $key + 1 ?>" type="number" min="0" value="<?= $orderItem['count'] ?>" required oninput="products.changeCost(event)" style="width: 100px"></td>
                    <td><input id="onStock<?= $key + 1 ?>" value="<?= $orderItem['stock']?>" disabled></td>
                    <td><input id="price<?= $key + 1 ?>" value="<?= $orderItem['price']?>"  disabled></td>
                    <td><input id="discount<?= $key + 1 ?>" name="discount_<?= $key + 1 ?>" value="<?= $orderItem['discount'] ?>" onchange="products.setStockAndPrice()" ></td>
                    <td><input class="sum" id="sum<?= $key + 1 ?>" name="sum_<?= $key + 1 ?>" value="<?= $orderItem['cost'] ?>" readonly></td>
                </tr>
            <?php endforeach; ?>
            <?php else: ?>
            <tr id="1">
                <th>1</th>
                <td><select class="product" id="product1" name="product_1" required onchange="products.setStockAndPrice(event)">
                        <option>Выберите товар</option>
                        <?php foreach ($products as $product): ?>
                ...
                <td><input class="product" id="count1" name="count_1" type="number" min="0" required oninput="products.changeCost(event)"></td>
